...
<script setup>
		...
		if (isValid) {
			if (data.pagenation.value === 0) {
				if (await data.email.existsUser()) {
					toast.error(`O email ${data.email.value} já está cadastrado!`)
	...
	const verifyCredentials = async () => {
		...
		const datas = await $fetch(data.url, {
			headers: {
				'content-type': 'application/json',
			},
			method: 'POST',
		...
		// await toast.success(`${data.response.email} cadastrado com sucesso!`)
		// toast.warning('Verifique sua caixa de entrada para ativar sua conta agora!')
		// } catch (error) {
	...
	// 	const url = `${data.urls.isExists}${data.email.value}`
	// 	console.log(url)
	// return await $fetch(`${data.urls.isExists}${data.email.value}`, {
	// 	headers: {
	// 		'content-type': 'application/json',
	// 	},
	// })
