<template>
  <form class="container" @submit.prevent="onSubmit">
    ...
    <div class="mb-3" v-else-if="data.pagenation.value === 1">
      <label for="input-password" class="form-label">{{ data.password.label }}</label>
      <div class="d-flex">
        <input
          ref="inputPassword"
          id="input-password"
        ...
        />
        <button type="button" class="mx-2" @click="data.icons.switchPasswordvisibility">
      ...
      <div class="d-flex justify-content-end">
        <button type="button" class="btn btn-secondary mx-2" @click="changePreviousPage">
          Voltar
        </button>
        <button
          type="button"
    ...
    <div class="mb-3" v-else-if="data.pagenation.value === 2">
      ...
      <div class="d-flex">
        <input
          ...
          @keyup.enter="changeNextPage(data.repeatPassword.isValid, inputRepeatPassword)"
        />
        <button type="button" class="mx-2" @click="data.icons.switchPasswordvisibility">
          <Icon :name="data.icons.current" :size="data.icons.size" />
        </button>
      ...
      <div class="d-flex justify-content-end">
        ...
        <button
          ...
          @click="changeNextPage(data.repeatPassword.isValid, inputRepeatPassword)"
        >
          Quase lá...
        </button>
      </div>
...
const data = reactive({
  ...
  email: {
    ...
    existsUser: async () => {
      ...
      const response = await $fetch(`${data.urls.isExists}${data.email.value}`, {
        headers: {
  ...
  repeatPassword: {
    ...
    isValid: false,
    placeHolder: "",
    describedby: "A repetição da senha tem que ser igual ao informado na senha.",
    focus: () => {
...
watch(data, async (newValue) => {
  ...
  email.regex.test(email.value) ? (email.isValid = true) : (email.isValid = false);
